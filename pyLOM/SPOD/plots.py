...
def plotMode(Phi, omega, xyz,mesh,info,dim=0,modes=np.array([1],np.int32),scale_freq=1.,fig=[],ax=[],cmap=None):
	...
	for imode, mode in enumerate(modes):
		...
		if len(ax) < imode + 1:
			ax.append( fig[imode].subplots(1,1,gridspec_kw = {'hspace':0.5}) )
		fig[imode].suptitle('Mode %d St = %.3f' % (mode-1, np.abs(omega)/(2*np.pi)))
		if mesh['type'] == 'struct2D':
			c = None
			if info['point']:
				c1 = plotFieldStruct2D(ax[imode],mesh['nx'],mesh['ny'],info['ndim'],xyz,Phi[:,mode-1],dim-1,cmap)
			else:
				xyzc = mesh_compute_cellcenter(xyz,mesh)
				c1 = plotFieldStruct2D(ax[imode],mesh['nx']-1,mesh['ny']-1,info['ndim'],xyzc,Phi[:,mode-1],dim-1,cmap)
			cf.append(c)
		plt.colorbar(mappable = c1, ax=ax[imode])
		ax[imode].set_title('Real mode')
		ax[imode].set_xlabel('x/D')
		ax[imode].set_ylabel('y/D')
		ax[imode].set_aspect('equal')
	return fig, ax, cf
