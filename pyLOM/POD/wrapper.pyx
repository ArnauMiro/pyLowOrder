...
@cython.nonecheck(False)
@cython.cdivision(True)    # turn off zero division check
def run(real[:,:] X, int remove_mean=True, int randomized=False, int r=1, int q=3, seed=None):
	'''
	Run POD analysis of a matrix X.
	...
	if real is double:
		return _drun(X,remove_mean, randomized, r, q, seed2)
	else:
		return _srun(X,remove_mean, randomized, r, q, seed2)
