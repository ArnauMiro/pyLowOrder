...
class TargetShapeWrapper(Dataset):
    """
    A dataset wrapper used to train GNNs that ensures that targets (Y) have shape [B, N, F],
    where:
        - B = batch size
        - N = number of graph nodes (fixed and known)
        - F = number of features per node

    This wrapper applies automatic reshaping in safe, unambiguous cases:
        - [N]         → [1, N, 1]
        - [N, F]      → [1, N, F]
        - [B, N]      → [B, N, 1]
        - [B, N, F]   → unchanged

    Ambiguous shapes like [F] or [B, F] will raise errors.

    Args:
        base_dataset (Dataset): Dataset yielding (X, Y) pairs (if only X is available, it will return X).
        num_nodes (int): Number of nodes in the graph (N)
    """
    def __init__(self, base_dataset: Dataset, num_nodes: int):
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(item, (tuple, list)) and len(item) == 2:
            ...
            return item  # inputs-only dataset

        if not isinstance(y, torch.Tensor):
            raise TypeError(f"Target must be a torch.Tensor, got {type(y)}")

        if y.ndim == 1:
            if y.shape[0] != self.num_nodes:
                raise ValueError(f"Expected 1D target with {self.num_nodes} elements, got {y.shape}")
            y = y.unsqueeze(0).unsqueeze(-1)  # [1, N, 1]

        elif y.ndim == 2:
            if y.shape[0] == self.num_nodes:
                y = y.unsqueeze(0)  # [1, N, F]
            elif y.shape[1] == self.num_nodes:
                raise ValueError(f"Ambiguous target shape {y.shape}. Did you transpose?")
            else:
            ...
            raise ValueError(f"Unsupported target shape: {y.shape}")

        assert y.shape[1] == self.num_nodes, f"Expected N={self.num_nodes} nodes, got shape {y.shape}"
