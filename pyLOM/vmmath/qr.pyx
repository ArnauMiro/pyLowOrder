...
from .cfuncs       cimport c_sqr, c_stsqr, c_srandomized_qr, c_slocal_randomized_qr, c_sinit_randomized_qr, c_supdate_randomized_qr
from .cfuncs       cimport c_dqr, c_dtsqr, c_drandomized_qr, c_dlocal_randomized_qr, c_dinit_randomized_qr, c_dupdate_randomized_qr
from .cfuncs       cimport c_cqr, c_ctsqr 
from .cfuncs       cimport c_zqr, c_ztsqr 
...
@cython.nonecheck(False)
@cython.cdivision(True)    # turn off zero division check
def _srandomized_qr(float[:,:] A, int r, int q, int seed, bint local):
	'''
	Parallel Randomized QR factorization using Lapack.
		Q(m,r)   
		B(r,n)  
	...
	cdef np.ndarray[np.float32_t,ndim=2] B = np.zeros((r,n),dtype=np.float32)
	# Compute SVD using randomized algorithm
	retval = c_slocal_randomized_qr(&Q[0,0],&B[0,0],&A[0,0],m,n,r,q,seed) if local else c_srandomized_qr(&Q[0,0],&B[0,0],&A[0,0],m,n,r,q,seed)
	if not retval == 0: raiseError('Problems computing Randomized SVD!')
	return Q,B
...
@cython.nonecheck(False)
@cython.cdivision(True)    # turn off zero division check
def _drandomized_qr(double[:,:] A, int r, int q, int seed, bint local):
	'''
	Parallel Randomized QR factorization using Lapack.
		Q(m,r)   
		B(n,n)    
	...
	cdef np.ndarray[np.double_t,ndim=2] B = np.zeros((r,n),dtype=np.double)
	# Compute SVD using randomized algorithm
	retval = c_dlocal_randomized_qr(&Q[0,0],&B[0,0],&A[0,0],m,n,r,q,seed) if local else c_drandomized_qr(&Q[0,0],&B[0,0],&A[0,0],m,n,r,q,seed)
	if not retval == 0: raiseError('Problems computing Randomized SVD!')
	return Q,B
...
@cython.nonecheck(False)
@cython.cdivision(True)    # turn off zero division check
def randomized_qr(real[:,:] A, const int r, const int q, const int seed=-1, const bint local=False, const bint hybrid=False):
	'''
	Parallel Single value decomposition (SVD) using Lapack.
		Q(m,r)   
		B(n,r)   
	...
	seed = <int>time(NULL) if seed < 0 else seed
	if real is double:
		return _drandomized_qr(A,r,q,seed,local)
	else:
		return _srandomized_qr(A,r,q,seed,local)
