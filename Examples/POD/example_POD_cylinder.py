...
X = cp.array(d[VARIABLE])
t = d.get_variable('time')


## Run POD
PSI,S,V = pyLOM.POD.run(X,remove_mean=False) # PSI are POD modes
#if pyLOM.utils.is_rank_or_serial(root=0): pyLOM.POD.plotResidual(S.get())
# Truncate according to a residual
PSI,S,V = pyLOM.POD.truncate(PSI,S,V,r=5e-6)
pyLOM.POD.save('results.h5',PSI.get(),S.get(),V.get(),d.partition_table,nvars=2,pointData=d.point)
...
## Dump to ParaView
# Spatial modes
d.add_field('spatial_modes_U',6,pyLOM.POD.extract_modes(PSI.get(),1,len(d),modes=[1,4,6,2,5,3]))
d.add_field('spatial_modes_V',6,pyLOM.POD.extract_modes(PSI.get(),2,len(d),modes=[1,4,6,2,5,3]))
