...
def convert_dataset(dset: str, datapath: str, npoints: int, ptable: pyLOM.PartitionTable) -> None:
    pyLOM.pprint(0,f"[{dset.upper()}] Processing dataset")
    ...
    for ii, (c, M, AoA) in enumerate(zip(case, Mvec, AoAvec), total=len(Mvec), desc=f"[{dset}]"):
    ...
    d.save(out_path)
    pyLOM.pprint(0,f"[{dset.upper()}] Saved to {out_path}\n")
    return d
...
def create_graph(datapath: str, npoints: int) -> pyLOM.NN.Graph:
    ...
    g = pyLOM.NN.Graph(edge_index=edge_index, node_features_dict=node_features_dict, edge_features_dict=edge_features_dict)
    pyLOM.pprint(0,"Graph created.")
    return g
