...
#
# PYLOM Testsuite
# Run BinaryClassifier on the synthetic dataset
#
...
## Build synthetic binary labels from the continuous field (median split)
cp     = y[:,0]
labels = (cp > np.quantile(cp, 0.5)).astype(np.float64)


## Load pyLOM dataset and set up results output
RESUDIR = os.path.join(OUTDIR,f'BinaryClassifier_{DATAFILE}')
...
pipeline = pyLOM.NN.Pipeline(
    ...
    model_class=pyLOM.NN.BinaryClassifier,
    optimizer=optimizer,
)
...
model = pyLOM.NN.BinaryClassifier.load(RESUDIR + "/model.pth", verbose=False)
preds, trues = model.predict(td_test, return_targets=True)


## Performance check: fraction of misclassified test points
pred_classes = (preds >= 0.5).astype(np.float64)
