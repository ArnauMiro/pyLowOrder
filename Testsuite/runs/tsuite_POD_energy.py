...
pyLOM.POD.save(f'{OUTDIR}/results.h5',PSI,S,V,d.partition_table,nvars=2,pointData=d.point)
...
## Testsuite output
M = pyLOM.POD.extract_modes(PSI,1,len(d),modes=[1,2,3])
...
## Dump to ParaView
# Spatial modes
d.add_field('spatial_modes',3,pyLOM.POD.extract_modes(PSI,1,len(d),modes=[1,2,3]))
...
if pyLOM.utils.is_rank_or_serial(0):
	...
	fig, _ = pyLOM.POD.plotMode(V[:,:-1],t[:-1],modes=[1,2,3])
